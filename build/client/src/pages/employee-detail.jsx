...
var EmployeeDetail = function () {
    var _a = useRoute("/employees/:id"), params = _a[1];
    var userId = parseInt((params === null || params === void 0 ? void 0 : params.id) || "0");
    var _b = useQuery({
        queryKey: ["/api/users/".concat(userId)],
    }), user = _b.data, isLoading = _b.isLoading;
    var tasks = useQuery({
        queryKey: ["/api/tasks/user/".concat(userId)],
    }).data;
    var attendance = useQuery({
        queryKey: ["/api/attendance"],
    }).data;
    var userAttendance = attendance === null || attendance === void 0 ? void 0 : attendance.filter(function (a) { return a.userId === userId; });
    var today = new Date().toISOString().split('T')[0];
    var checkedInToday = userAttendance === null || userAttendance === void 0 ? void 0 : userAttendance.some(function (a) { return a.date === today; });
    var activeTasksCount = (tasks === null || tasks === void 0 ? void 0 : tasks.filter(function (t) { return t.status === "in_progress"; }).length) || 0;
    var completedTasksCount = (tasks === null || tasks === void 0 ? void 0 : tasks.filter(function (t) { return t.status === "completed"; }).length) || 0;
    var overdueTasksCount = (tasks === null || tasks === void 0 ? void 0 : tasks.filter(function (t) {
    ...
    if (!user) {
        ...
        <Button className="mt-4" onClick={function () { return window.history.back(); }}>Go Back</Button>
      </div>);
    }
      ...
      <div className="flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <div className="h-16 w-16 overflow-hidden rounded-full">
            <img src={user.profileImageUrl || "https://ui-avatars.com/api/?name=".concat(user.firstName, "+").concat(user.lastName)} alt={"".concat(user.firstName, " ").concat(user.lastName)} className="h-full w-full object-cover"/>
          </div>
          <div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <span className={"inline-flex rounded-full px-2 py-1 text-xs font-medium ".concat(checkedInToday
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {tasks && tasks.length > 0 ? (tasks.slice(0, 3).map(function (task) { return (<TaskCard key={task.id} task={task} users={[user]}/>); })) : (<p className="text-center text-sm text-neutral-500">No tasks assigned to this employee.</p>)}
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="tasks" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {tasks && tasks.length > 0 ? (tasks.map(function (task) { return (<TaskCard key={task.id} task={task} users={[user]}/>); })) : (<p className="text-center text-sm text-neutral-500">No tasks assigned to this employee.</p>)}
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="attendance">
          <Card>
            ...
            <CardContent>
              {userAttendance && userAttendance.length > 0 ? (<div className="space-y-4">
                  {userAttendance.map(function (record) { return (<div key={record.id} className="flex items-center justify-between rounded-md border border-neutral-200 p-3 dark:border-neutral-700">
                      <div>
                        ...
                        <div className="flex gap-2 text-sm text-neutral-500">
                          ...
                          {record.checkOutTime && (<span>
                              Check out: {format(new Date(record.checkOutTime), "h:mm a")}
                            </span>)}
                        </div>
                      </div>
                      <span className={"inline-flex rounded-full px-2 py-1 text-xs font-medium ".concat(record.status === "active"
                    ? "bg-success-100 text-success-800 dark:bg-success-900 dark:text-success-300"
                    : record.status === "break"
                ...
                </div>) : (<p className="text-center text-sm text-neutral-500">No attendance records found.</p>)}
            </CardContent>
          </Card>
        </TabsContent>
